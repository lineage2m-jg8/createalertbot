...
async def LadderFunc(itemkind, number, ladderlist, channelVal):
	if number < len(ladderlist):
		...
		result_itemkindSTR = ','.join(map(str, itemkind))
		embed = discord.Embed(
			title = "----- 당첨! -----",
		...
		await MakeSound('축하합니다.'+result_ladderSTR+'님이.'+result_itemkindSTR+'에' + '당첨되었습니다.', './sound/say')
	...
	async def on_message(msg):
		...
		if client.get_channel(channel) != msg.channel:
			...
			if message.content == command[0]:
				...
				command_list += command[11] + ' [아이템] [뽑을인원수] [아이디1] [아이디2]...\n'     #!사다리
